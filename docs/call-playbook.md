...
| 단계 | 주도자 | 필수 저작 노드 dispatch | 필수 순수 자문 (읽기전용) |
|------|-------|-----------------------------|-------------------------|
...
| 자문가 (순수 자문 중 문제 발견) | 자문 요청자 | 요청자의 직속 상위 | PM |
...
### 에스컬레이션 포맷 (모든 저작 노드 dispatch 종료 시)
...
| 감리 단계 도래 | (사용자에게 안내) | PM 이 scripts/run_audit.sh 로 worktree 격리 후 general-purpose + audit-team 페르소나 dispatch | 감리 범위 |
...
### 5-4. business-manager (자문 중심, 저작 노드 dispatch 주체 아님)
...
QA 는 주로 읽기전용 자문 대상. 스스로 저작 노드 dispatch 불가.
