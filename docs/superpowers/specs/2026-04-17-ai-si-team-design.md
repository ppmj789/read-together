...
- subprocess 내부에서 다시 `claude -p --append-system-prompt ...` 호출 가능 → 다단 중첩 자유.
...
사용자가 직접 실행 (PM 을 통한 자동 dispatch 아님):
...
```bash
( claude -p --append-system-prompt "$(cat .claude/roles/backend-developer.md)" \
    --model sonnet --effort high --dangerously-skip-permissions \
    "<지시1>" > /tmp/<unique1>.log 2>&1 & \
  claude -p --append-system-prompt "$(cat .claude/roles/web-developer.md)" \
    --model sonnet --effort high --dangerously-skip-permissions \
...
| 15 | 감리 격리는 `git worktree` + Track A | 프롬프트·hook 차단보다 물리 격리가 간결·확실 |
