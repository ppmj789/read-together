...
- 핵심 참조 문서
  ...
  - `docs/call-playbook.md` — 역할별 호출 계약 정본 (drift-guard 대상)
  - `docs/superpowers/specs/2026-05-16-no-claude-p-ledger-redesign-design.md` — **현행 설계서** (Agent 단일 primitive + ledger 재설계). `2026-04-17-ai-si-team-design.md` + `2026-04-18-amendment-01-claude-p-invocation.md`(Amendment #1)는 claude -p 호출 부분이 2026-05-16 재설계로 대체됨.
  - `docs/superpowers/plans/2026-05-16-no-claude-p-ledger-redesign.md` — **현행 실행 플랜**
  - `templates/stage-gates.md` — 단계 진입·종결 조건 정본

## 저장소 구조

- `.claude/roles/<role>.md` — 역할 페르소나 단일 소스 (21 개)
- `.claude/agents/<role>-<variant>.md` — 자문 서브에이전트 shell (46 개)
- `.claude/skills/project-manager/SKILL.md` — PM Skill (SessionStart hook 자동 로드)
- `scripts/` — 14 개 validator / helper / bootstrap 스크립트
...
## 호출 계약 (call-playbook §0)

`claude -p` subprocess 는 사용하지 않는다. "Track A" 개념 폐기. 모든
위임·저작·자문은 현 세션 **Agent 툴 단일 primitive**:

- **저작 노드**: PM 이 `subagent_type=general-purpose` + 페르소나
  (`.claude/roles/<role>.md`) 프롬프트 주입 + `model` 티어로 dispatch.
  노드는 자기 산출물·자기 ledger 노드를 직접 Write, PM 반환은 노드
  경로·status·NEXT 요약만 (call-playbook §0-1).
- **순수 자문(읽기전용)**: `subagent_type=<role>-<variant>`
  (`.claude/agents/`, Read/Glob/Grep) dispatch (§0-2).
- **PM**: Skill 전용 최상위 세션. 모든 hop 필수 버스 + 공유 파일 단독
  scribe. general-purpose 는 Agent 툴 미보유 → 자율 중첩 불가 (§0-3).

## 공유 파일 단독 수정 규칙 (spec §7-2)

다음 파일은 PM(또는 영역의 단일 상위 에이전트)만 직렬 수정한다.
general-purpose 노드는 공유 파일 직접 수정 금지 (필요 시 ledger RESPONSE 에
명시 → PM 반영):
...
## Ledger 위임·통신 체계 (spec 2026-05-16)

위임·의사소통은 `projects/<name>/ledger/` 계층 트리에 축적된다. 노드
ID = Dewey(`A` → `A-1` → `A-1-1`), 요청-응답 완결 단일 문서
(`## REQUEST`/`## RESPONSE`/`## CHILD INDEX`/`## NEXT`), 실산출물
(00~05)·RTM-ID 를 링크하는 래퍼. 단계 종결 전 PM 이
`python3 scripts/validate_ledger.py projects/<name>` 통과 확인 (비0 →
종결 보류). 스키마·소유 규칙은 spec §2 / call-playbook §0-4.
...
두 명령이 모두 clean 이어야 commit (ledger 검증은 해당 프로젝트 작업 시 추가). 실패 시 근본 원인을 고치고 재실행
(`--no-verify` 같은 우회 금지).
...
- **공유 파일 단독 수정 규칙**, **`--add-dir` 범위 한정** (구 Track A 체계 — 2026-05-16 `claude -p` 폐기로 대체), **Track A CLI 순서** (구 Track A 체계 — 2026-05-16 `claude -p` 폐기로 대체), **CR 사이클 메타데이터**, **audit finding Type A/B/C/D 분류**, **4-계층 Track A depth guard** (구 Track A 체계 — 2026-05-16 `claude -p` 폐기로 대체), **`reviewed-by` 필수 기재** 등.
...
- `application/` — application-architect (overview·domain-model·business-
  ...
  Agent dispatch (구 "Track A dispatch" — 2026-05-16 `claude -p` 폐기로 대체).
- `technology/` — technical-architect (overview·middleware·deployment-
  topology·nfr-technology + ADR). 좁은 인프라·런타임·미들웨어·토폴로지
  한정. infrastructure-director 가 Agent dispatch (구 "Track A dispatch" — 2026-05-16 `claude -p` 폐기로 대체).
...
**신규 페르소나 `policy-engineer`** (Fixed Opus·xhigh) — PM dispatch 전용
(구 "Track A only" — 2026-05-16 `claude -p` 폐기로 Agent 툴 dispatch 로 대체).
WBS Validation 직후 사용자 합의 hook 명세를 받아
...
3. PM 이 합의 명세를 prompt 로 묶어 `policy-engineer-opus` Agent dispatch (구 Track A — 2026-05-16 `claude -p` 폐기로 대체)
...
코드 직접 수정 금지 — 변경은 policy-engineer Agent dispatch 재호출 (구 "Track A 재호출" — 2026-05-16 `claude -p` 폐기로 대체).
...
- **§1 Think Before Coding**: 가정·해석 분기·모호점을 명시 표면화. 내부
  추론으로 조용히 한 가지를 선택 금지. 모호함이 남으면 자문 서브에이전트
  dispatch 우선, 미해소 시 ESCALATION.
