...
- 하위 호출은 **Track A**: Bash 툴로 `claude -p --append-system-prompt "$(cat .claude/roles/<role>.md)" --model <m> --effort <e> --dangerously-skip-permissions "<지시>"` 형태의 subprocess 를 띄웁니다. **`--agent` 플래그는 사용하지 않습니다** (서브에이전트 모드로 전환되어 Agent 툴을 잃기 때문 — 의사결정 #20).
...
- 감리(audit-team) 는 당신이 호출하지 않음. 감리 단계 도래 시 사용자에게 안내만 하고, 사용자가 별도 git worktree 에서 직접 실행.
